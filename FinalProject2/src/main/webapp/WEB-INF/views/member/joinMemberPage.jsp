...
<script>
	function joinMemberSubmit(){
		...
		if(!regExp.test($("#inputId").val())){
			alert("아이디는 영문 소문자로 시작해야되고 총 4자리에서 20자 사이인 영문 숫자 조합으로 해야됩니다.");
			$("#inputId").focus();
			return;
		}
		...
		if(!regExp.test($("#inputPw").val())){
			alert("비밀번호는 최소 8 자 최대 12자, 소문자와대문자로 시작해야되고 특수문자 대소문자 숫자 조합해야됩니다.");
			$("#inputPw").focus();
			return;
		}
		*/
...
<body>
	...
	<form id="frm1" action="./joinMemberProcess" method="post">
		...
		<input type="button" value="회원가입" onclick="joinMemberSubmit2()">
	</form>
